 ...
 * @var string
 */
	private $required = array('mid', 'portalid', 'aid','portal_key', 'api_url', 'language');

   	public function __construct($parameters) {
     	$this->config_params=$parameters;
   	}
 ...
 */
	public function initialize(){
		$defaults=$this->config_params['payone'];
...
/**
 * Get All Countrues in a variable.
 * @return array
 */
...
/**
 * Sets Walet data for later processing.
 *
 * @param string $wallettype ie. PPT
 * @param string $successurl ie. http://exmple.com/success
 * @param string $errorurl  http://exmple.com/error
 * @param string $backurl ie. http://exmple.com
 * @return object
 */
	function setEWallet($wallettype, $successurl, $errorurl, $backurl) {
		$this->wlt = compact('wallettype', 'successurl', 'errorurl','backurl');
		return $this->wlt;
	}
 ...
 * Sets Creditcard data for later processing.
 *
 * @param string $onlinebanktransfertype ie. PNT
 * @param string $bankaccount ie. 2599100003
 * @param string $bankcode  ie. 12345678
 * @param string $iban ie. DE85123456782599100003
 * @param string $bic ie. TESTTEST
 * @return object
 */

	function setBA($onlinebanktransfertype, $bankaccount, $bankcode, $iban, $bic){
		$this->sb = compact('onlinebanktransfertype', 'bankaccount', 'bankcode', 'iban','bic');
		return $this->sb;
	}
 ...
 */
	function setPerson($d){
		if(!empty($d['country']) && !empty($d['firstname']) && !empty($d['lastname'])){
			$this->person = $d;
		} else {
			die('Set Person - Missing Details');
		}
	}
 ...
 * @return void
 */
	function setDeliveryData($d){
		if(!empty($d['shipping_firstname']) && !empty($d['shipping_lastname']) && !empty($d['shipping_street'])){
			$this->delivery = $d;
		} else {
			die('Set Person - Missing Details');
		}
 ...
 */
	function creditcardcheck() {
		$request = $this->am($this->cc, array('request' => 'creditcardcheck','storecarddata'=>'yes'));
		$this->response = $this->sendRequest($request);
		if($this->response['status'] == 'VALID'){
			return array('status'=>'success', 'code'=>$this->response['truncatedcardpan']);
		} else if ($this->response['status']=='ERROR' || $this->response['status']=='INVALID') {
			return array('status'=>'error', 'customermessage'=>$this->response['customermessage']);
		}
		
	}

/**
 * Validates Bank Account data based on prior setBA().
 * @return boolean true on success
 */
	function bankaccountcheck() {
		$request = $this->am($this->sb, array('request' => 'bankaccountcheck','checktype'=>'0'));
		$this->response = $this->sendRequest($request);
		if($this->response['status'] == 'VALID'){
			return array(
				'status'=>'success', 
				'bankcountry'=>$this->response['bankcountry'],
				'iban'=>$this->response['iban'],
				'bic'=>$this->response['bic']
			);
		} else if ($this->response['status']=='ERROR' || $this->response['status']=='INVALID') {
			return array('status'=>'error', 'customermessage'=>$this->response['customermessage']);
		}
 ...
 */
	function authorization($reference, $amount, $currency="USD", $narrative_text="", $clearingtype='cc'){
		...
		//Switch Clearing Type
		switch($clearingtype){
			//Credit Cart Payments
			case 'cc' :
				if(!empty($this->cc)){
					$request = $this->am($this->cc, $request);
				}

				if(!empty($this->person)){
					$request = $this->am($this->person, $request);
				}

				if(!empty($this->invoice)){
					$request = $this->am($this->invoice, $request);
				}

				if(!empty($this->delivery)){
					$request = $this->am($this->delivery, $request);
				}

				if(!empty($this->article)){
					$request = $this->am($this->article, $request);
				}
				$this->response = $this->sendRequest($request);
				break;
			//Bank Payments
			case 'sb':
				if(!empty($this->sb)){
					$request = $this->am($this->sb, $request);
				}
				...
				$this->response = $this->sendRequest($request);
				break;
			//Walet(PayPal) Payments
			case 'wlt':
				if(!empty($this->wlt)){
					$request = $this->am($this->wlt, $request);
				}
